...
HelloGitHub 推出的[《讲解开源项目》](https://github.com/HelloGitHub-Team/Article)系列，今天给大家带来一款开源 Java 版网盘项目—— [kiftd-source](https://github.com/KOHGYLW/kiftd-source)，网盘我想你值得拥有一个，本编通过3分钟带领大家搭建个人网盘。
...
### 2.1 Windows环境运行
**下载安装包**<br/>
直接从官网下载最新的安装包，安装地址：https://kohgylw.gitee.io/<br/>
项目比较温馨，支持三种下载方式：Github下载，阿里云下载，Gitee下载。如下图：
![](./images/1.png)
**检查配置**<br/>
...
admin.auth=cudrm
```
即 用户名:admin 密码：000000
这样登录之后就可以使用网盘的所有功能了。
## 2.2 Linux环境运行
**安装Screen工具**<br/>
Screen 工具能够虚拟出一个终端并执行相应的操作。因为本篇所讲的网盘需要一个终端。
执行如下命令安装 `Screen`:
```bash
yum install screen
```
...
### 3.2 运行
打开kohgylw.kiftd.mc.MC类，进行测试运行。这里注意：使用`Eclipse`工具打开。
![](./images/13.png)
...
### 4.5 在线预览
网盘支持文档 `txt`,`pdf`,`docx`，`ppt` 在线预览功能，支持图片的在线预览。
图片预览效果如下：
![](./images/10.png)
`pdf` 文件预览效果如下：
![](./images/11.png)
### 4.6 分享下载链接
网盘也考虑的文件的分享，它可以生成下载链接，浏览器访问下载链接就可以直接下载文件。选择需要下载的文件，点击 `下载` 按钮，选择`下载链接+`，既可以生成文件下载链接。如下图:
![](./images/12.png)

## 五、最后

这个网盘还不错吧！而且搭建也特别简单。一些重要的东西就可以存放到自己的网盘啦！说到底，编程语言只是工具，我们只要很好的使用工具，再加上自己天马行空的思想，我想会创造出更多不可思议的项目。
